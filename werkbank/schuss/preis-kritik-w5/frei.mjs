/* IST DIE TAFEL ERREICHBAR, WENN MAN VORHER AUFRAEUMT?
   Kopie von decke2.mjs, ein Unterschied: sie beantwortet erst die Blaetter
   der FUHRE und des ERBE und schlaegt dann die Michaelitafel auf.
...
const ZIEL = process.argv[4] || `/tmp/pk5/frei-e${ep}.json`;
...
for (let i = 0; i < WOCHEN; i++) {
  ...
  if (s.woche === 1) {
    ...
    /* DIE HAND, DIE ERST AUFRAEUMT. Was `decke2.mjs` gefunden hat: an jedem
       zweiten Michaeli liegen DIE FUHRE (`fuhre:uebergabe:nein` /
       `fuhre:ausgang:nein`) und DAS ERBE (`.erb-buch`) ueber der
       Michaelitafel. Ein Mensch beantwortet die zuerst. Diese Hand auch —
       und dann wird gemessen, ob die Tafel danach erreichbar ist. */
    for (let a = 0; a < 3; a++) {
      let was = false;
      if (await klick('fuhre:uebergabe:nein', 200)) was = true;
      if (await klick('fuhre:ausgang:nein', 200)) was = true;
      const zu = await seite.evaluate(() => {
        const b = [...document.querySelectorAll('.erb-buch [data-zug], [data-zug^="erbe:blatt-zu"], [data-zug="erbe:zu"]')]
          .find(x => /schlie|zurück|weiter/i.test(x.innerText || ''));
        if (!b || b.disabled) return null;
        const r = b.getBoundingClientRect();
        return r.width && r.height ? { x: r.left + r.width / 2, y: r.top + r.height / 2 } : null;
      });
      if (zu) { await seite.mouse.click(zu.x, zu.y); await ruhe(200); was = true; }
      /* Escape schliesst die Blaetter, die es anbieten (ZUSTAENDIGKEIT 2). */
      if (!was) break;
    }
