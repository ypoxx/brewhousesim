/* DER PREIS — DIE HAND, DIE FESTLEGUNGEN WILL (Welle 5, blinder Kritiker).
   Kopie von hand.mjs; nur die Wahl der Festlegung ist anders (siehe dort).
...
const ZIEL = process.argv[4] || `/tmp/pk5/fest-e${ep}.json`;
...
for (let i = 0; i < WOCHEN; i++) {
  ...
  if (s.woche === 1) {
    ...
    festAngeboten.push({ jahr: s.jahr, kasse: m.kasse,
    ...
    /* DIE HAND, DIE FESTLEGUNGEN WILL. Unterschied zur Hand daneben: sie
       nimmt nicht die BILLIGSTE unter 45 % der Kasse, sondern die TEUERSTE,
       die das Spiel ueberhaupt zulaesst (`data-soll-aus !== "1"`, nicht
       `disabled`) — und sie laesst die Angebote stehen, damit das Geld fuer
       die Festlegung da ist. Sie beantwortet die Frage, wie viele
       unwiderrufliche Festlegungen in vierzehn Jahren UEBERHAUPT zu haben
       sind, nicht wie viele eine vorsichtige Hand nimmt. */
    ...
    } else if (feste.length) {
      gewaehlt = feste.reduce((a, z) => (kosten(z.preis) > kosten(a.preis) ? z : a));
    }
