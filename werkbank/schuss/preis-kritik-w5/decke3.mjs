...
const ZIEL = process.argv[4] || `/tmp/pk5/decke3-e${ep}.json`;
  ...
  if (s.woche === 1) {
    ...
    const decke = await seite.evaluate(() => {
      ...
      document.querySelectorAll('[data-zug^="preis:festlege:"],[data-zug^="preis:nimm:"]').forEach(el => {
        ...
        /* WARUM nicht getroffen: der eigene Stil und der jedes Vorfahren,
           bis zur Ebene. Nur so laesst sich „liegt etwas darauf" von
           „ist selbst nicht anfassbar" unterscheiden. */
        const kette = []; let m = el;
        while (m && m.id !== 'buehne' && kette.length < 10) {
          const cs = getComputedStyle(m);
          const mr = m.getBoundingClientRect();
          kette.push({ wo: m.tagName.toLowerCase()
              + (m.id ? '#' + m.id : '')
              + (typeof m.className === 'string' && m.className ? '.' + m.className.trim().split(/\s+/).join('.') : ''),
            pe: cs.pointerEvents, vis: cs.visibility, op: cs.opacity, disp: cs.display,
            ov: cs.overflow, cl: cs.clipPath, tr: cs.transform.slice(0, 40),
            rect: [Math.round(mr.left), Math.round(mr.top), Math.round(mr.width), Math.round(mr.height)] });
          m = m.parentElement;
        }
        out.push({ zug: el.getAttribute('data-zug'), pfad: teile.join(' < '),
                   punkt: [Math.round(cx), Math.round(cy)], kette });
      });
