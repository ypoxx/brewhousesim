/* DER PREIS — Welle 5, Nacharbeit: dieselbe Linie, mehr mitgeschrieben.
   HAFEN=8899 node linie.mjs <epoche> <wochen> <ausgabe.json>

   ABSTAMMUNG: Byte fuer Byte `werkbank/schuss/preis-kritik-w5/linie-vorbild.mjs`
   = `werkbank/schuss/rueckkopplung-r3/linie.mjs` (ZUSTAENDIGKEIT 16 — am
   fremden Messgeraet wird nicht gedreht; das Vorbild liegt unangetastet
   daneben und liefert die rho-Zahlen dieses Berichts). Geaendert ist genau
   zweierlei, und beides aendert das SPIEL nicht:

   (1) Je Michaeli wird `BRAUHAUS.preis.taxe()` mitgeschrieben — die Taxe
       jeder Festlegungskarte, die Jahreslast, die Kasse und die Sichtlage
       jedes `preis:festlege:*`-Knopfes (`disabled` UND `data-soll-aus`).
       Das ist die Zahl, die dem Kritiker gefehlt hat: er konnte zaehlen,
       DASS jede Karte aus ist, aber nicht, WIE WEIT sie danebenliegt.
   (2) `WILL=1` tauscht die Festlegungsregel: statt der billigsten unter
       45 % der Kasse nimmt die Hand die TEUERSTE, die das Spiel zulaesst,
       und laesst dafuer die Angebote stehen (die Hand des Kritikers,
       `festhand.mjs`). Ohne `WILL` ist die Hand wortgleich das Vorbild.

   Gespielt wird sonst nicht anders. Das Warten (RUHE/BEHARR) ist
...
/* DIE TAXE, aufgeschluesselt — nur gelesen (`B.preis.taxe()`), dazu die
   Sichtlage jedes Siegelknopfes nach BEIDEN Lesarten (ZUSTAENDIGKEIT 25). */
async function taxe() {
  return await seite.evaluate(() => {
    let t = null;
    try { t = window.BRAUHAUS.preis.taxe(); } catch (e) { t = { fehler: String(e) }; }
    const knoepfe = [];
    document.querySelectorAll('[data-zug^="preis:festlege:"]').forEach(el => {
      const r = el.getBoundingClientRect();
      knoepfe.push({ zug: el.getAttribute('data-zug'),
        preis: el.getAttribute('data-preis') ? +el.getAttribute('data-preis') : null,
...
for (let i = 0; i < WOCHEN; i++) {
  ...
  if (s.woche === 1) {
    ...
    const vorTaxe = await taxe();

    let genommeneFest = null;
    if (WILL) {
      /* Die Hand, die Festlegungen WILL: teuerste zulaessige Karte, Angebote
         bleiben stehen, damit das Geld dafuer da ist. */
    ...
    taxen.push({ jahr: s.jahr, amtszeit: s.amtszeit, genommen: genommeneFest, ...vorTaxe });

    const ang = WILL ? [] : alle(m, /^preis:nimm:/).filter(z => z.preis);
    if (ang.length) {
