...
HAFEN=${1:-8951}
NAME=${2:-lauf}
ZIEL=werkbank/schuss/fuhre-w11/messungen
mkdir -p "$ZIEL"
export MESSFENSTER_WARTE=7200
for e in 1 2 3 4; do
  echo "== rho $NAME E$e  $(date -u +%H:%M:%S)"
  HAFEN=$HAFEN werkbank/schuss/aufsicht/messfenster.sh \
    node werkbank/schuss/rueckkopplung-r3/linie.mjs "$e" 400 "$ZIEL/rho-$NAME-e$e.json" \
    >> "$ZIEL/rho-$NAME.log" 2>&1
  echo "   -> $? $(date -u +%H:%M:%S)"
done
echo "== auswerten"
python3 werkbank/schuss/fuhre-w6/schnitte.py "$ZIEL"/rho-"$NAME"-e*.json > "$ZIEL/rho-$NAME-schnitte.txt" 2>&1
python3 werkbank/schuss/rueckkopplung-r3/auswerten.py "$ZIEL"/rho-"$NAME"-e*.json > "$ZIEL/rho-$NAME-jahre.txt" 2>&1
