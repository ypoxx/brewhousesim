...
const lage = (z) => seite.evaluate((zz) => {
  ...
  return { x, y, aus: !!k.disabled, trifft: !!(t && (t === k || k.contains(t))),
    drin, drauf: (function(){var e=t,k=[];while(e&&k.length<7){k.push(e.tagName+(e.id?'#'+e.id:'')+(e.className?'.'+String(e.className).trim().split(/\s+/).join('.'):''));e=e.parentElement;}return k.join(' < ');})(),
