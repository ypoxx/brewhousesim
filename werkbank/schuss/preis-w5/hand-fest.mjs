...
for (let i = 0; i < WOCHEN; i++) {
  ...
  if (s.woche === 1) {
    ...
    /* DIE FESTLEGUNGSWILLIGE HAND — der einzige Unterschied zu `hand.mjs`.

       Die Referenzhand nimmt eine Festlegung nur, wenn sie hoechstens 45 % der
       Lade kostet, und sie rechnet dabei `Math.abs(preis)` — ein Schild mit
       PLUS (die Festlegung bringt Geld herein) gilt ihr als ebenso teuer wie
       eines mit Minus. Das sind zwei Regeln der HAND, nicht des Spiels.
       Deshalb steht daneben diese zweite Hand: sie nimmt, was das Spiel
       hergibt — Zufluss zuerst, sonst die billigste, die nicht abgeschaltet
       ist. `aus` bleibt `aus`: was das Spiel sperrt, wird nicht angefasst. */
    const feste = alle(m, /^preis:festlege:/).filter(z => z.preis !== null);
    if (feste.length) {
      const herein = feste.filter(z => z.preis > 0);
      const b = herein.length
        ? herein.reduce((a, z) => (z.preis > a.preis ? z : a))
        : feste.reduce((a, z) => (Math.abs(z.preis) < Math.abs(a.preis) ? z : a));
