...
function umkaempft(s) {
  const kand = s.zuege.filter(z => !z.aus && z.preis !== null && z.preis < 0
    && /^gegner:(abloesen|zuvorkommen|abwehren|ueberbieten)/.test(z.zug));
...
for (let i = 0; i < WOCHEN; i++) {
  ...
  const umk = umkaempft(s);
  reihe.push({ n: i, jahr: s.jahr, woche: s.woche, kasse: s.kasse, kasseAmSchirm: s.kasseAmSchirm,
    umkDeckung: umk.wert === null ? null : +umk.wert.toFixed(3), umkZug: umk.was, umkPreis: umk.preis, umkZahl: umk.n,
...
const uu = reihe.filter(r => r.umkDeckung !== null);
if (uu.length) {
  const p = (a) => a.map(r => r.umkDeckung).sort((x,y)=>x-y);
  const teil = (a,q) => p(a)[Math.floor(p(a).length*q)] ;
  console.log(`  umkaempft: erste ${uu[0].umkDeckung}x (${uu[0].umkZug} ${uu[0].umkPreis}) letzte ${uu[uu.length-1].umkDeckung}x (${uu[uu.length-1].umkZug} ${uu[uu.length-1].umkPreis})`);
  console.log(`  umkaempft: min ${Math.min(...uu.map(r=>r.umkDeckung))} med ${teil(uu,0.5)} max ${Math.max(...uu.map(r=>r.umkDeckung))} n=${uu.length}`);
  const vier = Math.ceil(uu.length/4);
  for (let i=0;i<4;i++){ const t=uu.slice(i*vier,(i+1)*vier); if(!t.length) continue;
    console.log(`    Viertel ${i+1}: median ${teil(t,0.5)}x  Kasse ${Math.round(t.reduce((a,b)=>a+b.kasse,0)/t.length)}  Preis ${Math.round(t.reduce((a,b)=>a+b.umkPreis,0)/t.length)}`); }
}
