...
while (Date.now() < ENDE) {
  ...
  if (s.zuege.some(z => z.hit && !z.aus && (/^preis:(nimm|festlege):/.test(z.zug) || z.zug === 'preis:tafel-zu'))) {
    ...
    const kand = [...fest, ...nimm].filter(z => Math.abs(z.preis) * 2 <= s.kasse);
    if (kand.length) {
      const b = kand.reduce((a, z) => (Math.abs(z.preis) < Math.abs(a.preis) ? z : a));
      await greif(b.zug, { grund: 'Michaeli: billigstes, das ich mir zweimal leisten kann', warte: 300 });
    }
    await greif('preis:tafel-zu', { grund: 'Das Jahr beginnen', warte: 300 });
  }

  /* Rohstoff — sonst steht die Pfanne kalt */
  s = await schirm();
  if (s.rohstoff < 40) {
    const roh = s.zuege.find(z => /kauf:rohstoff/.test(z.zug) && !z.aus);
    if (roh && roh.preis && s.kasse >= 2.5 * Math.abs(roh.preis))
      await greif(roh.zug, { grund: 'Rohstoff geht aus', warte: 220 });
  }

  /* Brauplan aufstocken, wenn der Keller leer ist */
  s = await schirm();
  if (s.plaetze && s.faesser / s.plaetze < 0.55) {
    for (let k = 0; k < 3; k++) {
