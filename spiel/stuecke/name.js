...
(function (B) {
  ...
  function knopfFuer(t) {
    ...
    if (t.art === 'jahr' && an) { text = t.name.replace(/^(\w+)/, 'Einstellen:') + ''; }
    if (t.art === 'jahr' && an) { text = 'Einstellen: ' + t.name.replace(/ (auflegen|anschlagen lassen|mieten|ausgeben|bedrucken|senden)$/, ''); preis = 0; }
